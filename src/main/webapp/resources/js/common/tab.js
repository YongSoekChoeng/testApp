...
function addTab(tabID,Name,URL,MenuPath,reloadYN){ // 메뉴를 눌러야 호출됨.
  ...
  var ifm = "<iframe id='_"+tabID+"' name='_"+tabID+"'"+ // " src='"+URL+"'"
            ...
            " menuID='"+tabID+"'"+
            " style='height:100%;width:100%;border: 0px;overflow:hidden;min-height:750px;' "+
            " scrolling=\"no\" "+ 
            //" scrolling=\"yes\" "+ 
            " targetURL='"+URL+"'"+ ">" +
            "</iframe> ";
  ...
  if(tabID == 'MA_000'){ // 메인인경우 종료 버튼 없음
	  ...
	  $("#tablist").append('<a href="#" onClick="javascript:selectTab(\''+tabID+'\',\''+Name+'\',\''+MenuPath+'\')"><li class="t_nor" id="tab_'+tabID+'" onClick="javascript:selectTab(\''+tabID+'\',\''+Name+'\',\''+MenuPath+'\')">'+Name
...
function selectTab(tabID,Name,MenuPath) {
	...
	if(MenuPath.indexOf(">")!= -1){
		$("#image_menu_path").html('<img src="/resources/images/common/ic_help01.png" style="width:14px;" >'+ MenuPath.replace(/>/gi,'<img src="/resources/images/common/ic_8.png">'));
		//$("#image_menu_path").attr('onclick', 'javascript:helpItemView(\'' + tabID + '\', \'' + MenuPath + '\');');
	}else{
