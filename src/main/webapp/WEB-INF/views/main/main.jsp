...
<div id="wrap">
	<div class="new-wrapper" style="overflow-x:auto;height:97%;">
		<div class="content" style="padding: 0; margin-left: -10px; width: 105%">
			<div class="menu-trigger"></div>
			<div class="main_wrap" >메인 배경 이미지</div>
			<div class="main_sty01">
				<div class="main_sty02">
					<ul class="main_box01">
						<c:forEach items="${templateMstList }"  begin="0" end="2" var="list" varStatus="i">
						<li>
								...
								<c:choose>
									...
									<c:when test="${list.main_board_type eq 'MAIN_IMAGE' }">
										...
										<c:if test="${fn:length(requestScope[MainImageTemplate]) eq 4 }">
											...
											<img src="${requestScope[MainImageTemplate][1].file_url }" class="imgsty04" onclick="goLinkPage('${requestScope[MainImageTemplate][1].link_gbn}', '${requestScope[MainImageTemplate][1].link_url}')" style="cursor: pointer;">
												...
												<c:choose>
													<c:when test="${User.mb_id eq 'AT'}">
														...
														<table class="table_st1 bt0">
															...
															<tbody>
																<c:forEach items="${requestScope[MAIN_STATISTICS_LONGTERM] }" var="longTermList" varStatus="status">
																<tr>
														...
														<span class="ttxt01">(단위 : 천원, 건)</span>
															<div class="AT_table">
																<table class="table_st1">
															        ...
															        <tbody>
															        	<c:forEach items="${requestScope[MAIN_STATISTICS_SCD] }" var="scdRanklist">
																		<tr>
														...
														<table class="table_st1">
													        ...
													        <tbody>
																...
																<tr>
																	<td class="tal">${scdRanklist.snm }</td>
																	<td><fmt:formatNumber pattern="#,###">${scdRanklist.longterm_prem_amt / 1000 }</fmt:formatNumber></td>
																	<td class="br0"><fmt:formatNumber pattern="#,###">${scdRanklist.cont_longterm_cnt }</fmt:formatNumber></td>
																</tr>
																</c:forEach>
															</tbody>
														</table>
												...
												<c:choose>
													<c:when test="${User.mb_id eq 'AT'}">
														<table class="table_st1 bt0">
															...
															<tbody>
																<c:forEach items="${requestScope[MAIN_STATISTICS_LONGTERM] }" var="longTermList">
																<tr>
																	<td class="tal">${longTermList.snmpath3 }</td>
																	<td class="tal">${longTermList.emp_nm }</td>
																	<td><fmt:formatNumber pattern="#,###">${longTermList.share_longterm_prem_amt / 1000 }</fmt:formatNumber></td>
																	<td class="br0"><fmt:formatNumber pattern="#,###">${longTermList.cont_longterm_cnt }</fmt:formatNumber></td>
																</tr>
				...
				<div class="main_box02">
					<table class="table_st2">
						...
						<tbody>
							<c:forEach items="${EmpCodeList }" var="empCodeList" varStatus="i" >
								<tr>
									<th><img src="${empCodeList.ci_url }" class="imgsty_insco03" onclick="goInscoPage('${empCodeList.plan_url}')" style="cursor: pointer"></th>
									<td class="br0">
										ID : ${empCodeList.insco_emp_cd }<br/>
										헬프데스크 : <br/>${empCodeList.helpno }
									</td>
	      						</tr>
							</c:forEach>
						</tbody>
