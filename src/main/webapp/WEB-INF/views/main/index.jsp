		...
		<div id="headerwrap">
			<nav class="gb_gnb">
				<div class="fl">
					...
					${session.emp_nm} 님
					<span>(최종 로그인시간 ${session.login_time}) [<span onclick="javascript:sessionExt();" style="cursor: pointer; width: 80px; text-align: center; font-size: 14px;">
																<span id="sessionTimer" style="font-size: 14px;"></span>연장
															</span>]
					</span>
					<button id="changeEmp" class="gb01" onclick="changeEmp()">권한사원변경</button>
				</div>
				<div class="fr">
...
<!-- 					<div class="fl insucom" style="padding:0px;">
					...
					<button class="fl gb02" onclick="changeAccountPwd();">비밀번호 변경</button>
					<button class="fl gb02" onclick="Logout();">로그아웃</button>
				</div>
			</nav>
			<nav class="gnb">
				<a href="/index.go"><img src='/resources/images/common/logo_p.svg' ></a>
				<div id="topmenu"></div>
			</nav>
		</div><!-- // id="hederwrap" -->
	...
	<p class="breadcrumb" id="image_menu_path" style="cursor:pointer;z-index:1;">
		<!-- 인사관리  <img src="/resources/images/common/ic_8.png"> 보증관리  <img src="/resources/images/common/ic_8.png"> <span>보증보험</span> -->
	</p>
		...
		<div id="footer">
			<footer>${session.copy_right}</footer>
		</div>
	</div><!-- //id="container" -->
...
$(document).ready(function(){
	...
	for(var i=0; i<menuJSON.length; i++) {
		...
		if(r.lv == 1) {//최상위 메뉴						
			...
			if(r.isleaf == 1){
				...
				$("#"+r.prnt_resource_id).append("<a id='"+r.resource_id+"' href='#'>"+r.resource_name+"<img src='/resources/images/common/ic_3.png' class='fr'></a><ul id="+r.resource_id+"_"+r.lv+" ></ul>");
			}
    	}else if(r.lv == 3) {
		...
		if(r.lv != 1 && r.resource_url != "NONE" && r.resource_url != "") {
	...
	var loginRole = "${session.role_id}";
 	if(loginRole != "ROLE_0" && loginRole != "ROLE_1") {
